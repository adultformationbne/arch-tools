...
- **NEVER use `mcp__supabase__list_tables` without filtering** — it returns 14,000+ tokens and kills the context window. Instead, use targeted SQL queries filtered by table prefix (see CLAUDE.md for prefix groups).
...
- Schema calls can produce large payloads—use targeted SQL queries filtered by table prefix to keep responses manageable.
