	...
	<div class="mx-auto max-w-7xl">
		...
		<div class="mb-6 rounded-xl bg-white p-6 shadow-lg">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="mt-1 text-gray-600">Create structured content with custom tags</p>
				</div>
					...
					<div class="flex items-center gap-2 text-sm">
						...
						{:else if saveStatus === 'saved'}
							...
							<span class="text-green-600"
								>Saved to AHWGP_master.json {lastSaveTime
									? `at ${formatTime(lastSaveTime)}`
									: ''}</span
							>
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
			...
			<div class="rounded-xl bg-white p-6 shadow-lg">
				<h2 class="mb-4 text-xl font-semibold text-gray-800">Content Editor</h2>

				<!-- Current Selection -->
				<div class="mb-4 rounded-lg border border-purple-200 bg-purple-50 p-4">
					<div class="flex items-center justify-between">
						<div>
							<span class="font-medium text-purple-700">Selected Tag:</span>
							<span class="ml-2 font-bold text-purple-900">{selectedTag}</span>
						</div>
						{#if tagChangeMode}
							<div class="flex items-center gap-2 text-sm">
								<div class="h-3 w-3 animate-pulse rounded-full bg-orange-500"></div>
								<span class="font-medium text-orange-700">Hold Alt + Click to Change Tags</span>
							</div>
						{/if}
					</div>
					{#if !tagChangeMode}
				...
				<!-- Tag Selector -->
				<div class="mb-4">
					<h3 class="mb-2 text-sm font-medium text-gray-700">Select Tag</h3>
					<div class="flex flex-wrap gap-2">
						{#each allTags as tag}
							<button
								onclick={() => (selectedTag = tag)}
								class="rounded-full px-3 py-1.5 text-sm font-medium transition-all duration-200 {selectedTag ===
								tag
				...
				<!-- Metadata Selector -->
				<div class="mb-4">
					<h3 class="mb-2 text-sm font-medium text-gray-700">Add Metadata (Optional)</h3>
						...
						<div class="mt-2 flex flex-wrap gap-1">
							{#each selectedMetadata as meta}
								...
								>
									{meta}
									<button
										onclick={() => (selectedMetadata = selectedMetadata.filter((m) => m !== meta))}
										class="text-orange-500 hover:text-orange-700">×</button
									>
								</span>
							{/each}
						</div>
				...
				<div class="mb-4 flex gap-2">
					<input
						...
						class="flex-1 rounded-lg border border-gray-300 px-3 py-2 focus:border-transparent focus:ring-2 focus:ring-purple-500"
					/>
					<button
				...
				<div class="mb-4">
					<textarea
						...
						onpaste={handlePaste}
						placeholder="Enter your content here... {autoAddOnPaste
							? '(Paste auto-adds, or Ctrl+Enter to add)'
							: '(Ctrl+Enter to add)'}"
						class="h-32 w-full resize-none rounded-lg border border-gray-300 px-3 py-2 focus:border-transparent focus:ring-2 focus:ring-purple-500"
					></textarea>

					<!-- Auto-add toggle -->
					<div class="mt-2 flex items-center gap-2">
						<label class="relative inline-flex cursor-pointer items-center">
							<input type="checkbox" bind:checked={autoAddOnPaste} class="peer sr-only" />
							<div
			...
			<div class="rounded-xl bg-white p-6 shadow-lg">
				<div class="mb-4 flex items-center justify-between">
					...
					<button
						...
						class="flex items-center gap-2 rounded-lg bg-gray-100 px-3 py-1.5 text-sm transition-colors hover:bg-gray-200"
					...
					<div class="mb-4">
						<div class="flex flex-wrap items-center gap-2">
							<button
								...
								class="rounded-full px-3 py-1.5 text-sm font-medium transition-all duration-200 {!selectedChapter
							...
							{#if chapters.length > 5}
								<button
									...
									class="rounded-full bg-gray-200 px-3 py-1.5 text-sm font-medium text-gray-600 transition-all duration-200 hover:bg-gray-300"
								>
									{showAllChapters
										? `Hide ${chapters.length - 5} chapters`
				...
				{#if blocks.length === 0}
					<div class="py-12 text-center text-gray-500">
						...
						<p class="mt-2 text-sm">Start adding blocks to see them here</p>
					</div>
				{:else}
					<div class="max-h-[600px] space-y-2 overflow-y-auto">
						{#each filteredBlocks as block, index}
							<div
								class="rounded-lg border border-gray-200 p-3 transition-shadow hover:shadow-md {tagChangeMode
									? 'cursor-crosshair hover:bg-purple-50'
									: ''}"
								...
								<div class="flex items-start justify-between">
									<div class="flex-1">
										<div class="flex flex-wrap items-center gap-1">
											<span
												class="inline-block rounded px-2 py-1 text-xs font-medium {getTagColorClass(
													block.tag
												)}"
											>
												{block.tag}
											...
											{#if block.metadata && block.metadata.length > 0}
												{#each block.metadata as meta}
													<span
														class="inline-block rounded bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-700"
													>
														{meta}
													</span>
												{/each}
...
{#if showExportModal}
	<div class="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black p-4">
		<div class="max-h-[80vh] w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-xl">
			<div class="p-6">
				<h3 class="mb-4 text-2xl font-bold">Export Options</h3>

				<div class="space-y-4">
					<button
						onclick={downloadJSON}
						class="w-full rounded-lg bg-blue-600 px-4 py-3 text-left text-white transition-colors hover:bg-blue-700"
					>
					...
					<button
						onclick={downloadXML}
						class="w-full rounded-lg bg-green-600 px-4 py-3 text-left text-white transition-colors hover:bg-green-700"
					>
				...
				<div class="mt-6">
					<h4 class="mb-2 font-medium">XML Preview:</h4>
					<pre class="max-h-64 overflow-x-auto rounded-lg bg-gray-900 p-4 text-sm text-gray-100">
{xmlPreview}
					</pre>
				</div>
