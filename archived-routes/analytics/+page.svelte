			...
			return evaluations
				.filter((evaluation) => {
					...
					const searchMatch =
						...
						(scores &&
							scores.key_themes &&
							Array.isArray(scores.key_themes) &&
							scores.key_themes.some(
								(theme) =>
									theme &&
									typeof theme === 'string' &&
	...
	function exportFilteredData() {
		const data = {
			...
			evaluations: filteredEvaluations.map((evaluation) => ({
				...
				key_themes:
					evaluation.scores && evaluation.scores.key_themes ? evaluation.scores.key_themes : []
			}))
		};
		...
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			<div class="flex items-center justify-between py-6">
				...
				<div class="flex items-center space-x-4">
					...
					<select bind:value={viewMode} class="rounded-lg border border-gray-300 px-3 py-2">
				...
				<div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-4">
					...
					<div class="rounded-lg bg-white p-6 shadow">
						<div class="text-2xl font-bold text-purple-600">
							{(stats &&
								stats.metrics &&
								stats.metrics.relatability &&
								stats.metrics.relatability.avg) ||
								'N/A'}
						</div>
					...
					<div class="rounded-lg bg-white p-6 shadow">
						<div class="text-2xl font-bold text-orange-600">
							{(stats &&
								stats.metrics &&
								stats.metrics.accessibility &&
								stats.metrics.accessibility.avg) ||
								'N/A'}
						</div>
			...
			<div class="grid grid-cols-1 gap-8 lg:grid-cols-4">
				...
				<div class="lg:col-span-1">
					<div class="sticky top-4 rounded-lg bg-white p-6 shadow">
						<h3 class="mb-4 text-lg font-semibold text-gray-900">📊 Score Filters</h3>

						{#each Object.keys(filters) as metric}
							<div class="mb-6">
								<label class="mb-2 block text-sm font-medium text-gray-700 capitalize">
									{metric.replace('_', ' ')}
								...
								{#if stats?.metrics?.[metric]}
									<div class="mt-1 text-xs text-gray-500">
										...
										(Range: {stats?.metrics?.[metric]?.min || 'N/A'}-{stats?.metrics?.[metric]
											?.max || 'N/A'})
									</div>
								{/if}
							</div>
						...
						<div class="border-t pt-4">
							...
							<label class="mt-3 mb-2 block text-sm font-medium text-gray-700">Order</label>
							<select
				...
				<div class="lg:col-span-3">
										...
										<div class="flex-1">
											...
											<p class="leading-relaxed text-gray-800">
												{evaluation.content_full && typeof evaluation.content_full === 'string'
													? evaluation.content_full.substring(0, 200) + '...'
                                   ...
                                   bg-{recommendation.color}-100 text-{recommendation.color}-800"
									...
									<div class="grid grid-cols-3 gap-4">
										{#if evaluation.scores && typeof evaluation.scores === 'object'}
											{#each ['relatability', 'accessibility', 'emotional_resonance', 'theological_depth', 'practical_application', 'historical_focus'] as metric}
												...
												{#if score !== null}
													<div class="text-center">
														...
														<div
															class="text-lg font-semibold {getScoreColor(score)} rounded px-2 py-1"
														>
															{score}
														</div>
													</div>
												{:else}
													<div class="text-center">
														...
														<div
															class="rounded bg-gray-100 px-2 py-1 text-lg font-semibold text-gray-400"
														>
															N/A
														</div>
													</div>
											...
											{/each}
										{:else}
											<div class="col-span-3 text-center text-gray-500">No scores available</div>
										{/if}
									</div>
								</div>
							{/each}
										...
										<tr class="hover:bg-gray-50">
											<td class="px-6 py-4">
												<div class="max-w-md text-sm text-gray-900">
													{evaluation.content_full && typeof evaluation.content_full === 'string'
														? evaluation.content_full.substring(0, 100) + '...'
														: 'No content'}
												</div>
											</td>
											<td class="px-3 py-4 text-center">
												<span
													class="text-sm font-medium {getScoreColor(
														getSafeScore(evaluation, 'relatability')
													)} rounded px-2 py-1"
												>
											...
											<td class="px-3 py-4 text-center">
												<span
													class="text-sm font-medium {getScoreColor(
														getSafeScore(evaluation, 'accessibility')
													)} rounded px-2 py-1"
												>
											...
											<td class="px-3 py-4 text-center">
												<span
													class="text-sm font-medium {getScoreColor(
														getSafeScore(evaluation, 'emotional_resonance')
													)} rounded px-2 py-1"
												>
											...
											<td class="px-3 py-4 text-center">
												<span
													class="text-sm font-medium {getScoreColor(
														getSafeScore(evaluation, 'theological_depth')
													)} rounded px-2 py-1"
												>
											...
											<td class="px-3 py-4 text-center">
												<span
													class="text-sm font-medium {getScoreColor(
														getSafeScore(evaluation, 'practical_application')
													)} rounded px-2 py-1"
												>
											...
											<td class="px-3 py-4 text-center">
												<span
													class="text-sm font-medium {getScoreColor(
														getSafeScore(evaluation, 'historical_focus')
													)} rounded px-2 py-1"
												>
					...
					{:else if viewMode === 'charts'}
						...
						<div class="space-y-8">
							<div class="rounded-lg bg-white p-6 shadow">
								<h3 class="mb-4 text-lg font-semibold text-gray-900">📈 Score Distribution</h3>
								{#if stats}
									<div class="grid grid-cols-2 gap-6">
										{#each Object.entries(stats?.metrics || {}) as [metric, data]}
											<div>
												<h4 class="mb-2 text-sm font-medium text-gray-700 capitalize">
													{metric.replace('_', ' ')}
												</h4>
												<div class="flex h-24 items-end space-x-1">
							...
							<div class="rounded-lg bg-white p-6 shadow">
								<h3 class="mb-4 text-lg font-semibold text-gray-900">🎯 Content Recommendations</h3>
								<div class="grid grid-cols-3 gap-4">
									<div class="rounded-lg bg-green-50 p-4 text-center">
										<div class="text-3xl font-bold text-green-600">
											{recommendationCounts.keepCount}
										</div>
										<div class="text-sm text-green-700">Keep</div>
										<div class="text-xs text-green-600">
											{filteredEvaluations.length > 0
												? (
														(recommendationCounts.keepCount / filteredEvaluations.length) *
														100
													).toFixed(1)
												: 0}%
										</div>
									...
									<div class="rounded-lg bg-orange-50 p-4 text-center">
										<div class="text-3xl font-bold text-orange-600">
											{recommendationCounts.editCount}
										</div>
										<div class="text-sm text-orange-700">Edit</div>
										<div class="text-xs text-orange-600">
											{filteredEvaluations.length > 0
												? (
														(recommendationCounts.editCount / filteredEvaluations.length) *
														100
													).toFixed(1)
												: 0}%
										</div>
									...
									<div class="rounded-lg bg-red-50 p-4 text-center">
										<div class="text-3xl font-bold text-red-600">
											{recommendationCounts.removeCount}
										</div>
										<div class="text-sm text-red-700">Remove</div>
										<div class="text-xs text-red-600">
											{filteredEvaluations.length > 0
												? (
														(recommendationCounts.removeCount / filteredEvaluations.length) *
														100
													).toFixed(1)
												: 0}%
										</div>
