-- Fix feast readings priority: Fixed Feasts should use their proper readings, not ferial
-- Only if a feast has NO proper readings should we fall back to ferial
...
BEGIN
  ...
  -- PRIORITY 1: Try exact match on liturgical_day (for Solemnities and Fixed Feasts)
  -- All Fixed Feasts should use their proper feast readings if they exist
  RETURN QUERY
  SELECT
  ...
  -- PRIORITY 2: Try pattern matching on seasons (Advent, Lent, Easter, Christmas)
  ...
  -- PRIORITY 3: Try Ordinary Time ferial readings (fallback for optional memorials)
...
'Gets liturgical readings for a date. Prioritizes proper feast readings over ferial readings.
Fixed Feasts (St Mary Magdalene, St Patrick, etc.) use their specific feast readings.
Solemnities always use their proper readings.
Ferial readings are used as fallback only when no feast-specific readings exist.';
