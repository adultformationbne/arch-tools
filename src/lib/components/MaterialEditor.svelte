							...
							<div class="space-y-4 bg-white p-4 rounded-lg">
								...
								{:else if editingMaterial.type !== 'mux_video'}
									...
									{#if editingMaterial.type === 'link'}
										<div>
											<label
												for={getFieldId('edit-material-description', material.id)}
												class="block text-sm font-semibold text-gray-700 mb-1"
											>Description <span class="font-normal text-gray-500">(optional)</span></label>
											<textarea
												id={getFieldId('edit-material-description', material.id)}
												bind:value={editingMaterial.description}
												placeholder="Brief description of this link..."
												rows="2"
												class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none"
											></textarea>
										</div>
									{/if}
								{/if}
