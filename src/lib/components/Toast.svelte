<script>
  ...
  $: iconPath = iconPaths[toast.type] || iconPaths.info;
</script>
...
>
  <div class="flex items-start gap-3">
      ...
      {#if toast.steps && toast.totalSteps > 1}
        <div class="mt-3">
          ...
          <div class="w-full bg-black/10 rounded-full h-1.5">
            <div 
              class="bg-current rounded-full h-1.5 transition-all duration-300"
    ...
    {#if toast.closeable}
      <button
        onclick={handleClose}
        class="flex-shrink-0 ml-2 inline-flex text-current hover:opacity-70 focus:outline-none"
      >
  ...
  {#if toast.duration > 0}
    <div class="absolute bottom-0 left-0 h-1 bg-current opacity-20 transition-all duration-100"
