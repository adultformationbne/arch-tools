<script>
  ...
  const displayContent = $derived(isExpanded || !shouldTruncate ? block.content : truncateContent(block.content, 200));
...
<div class="border rounded-lg p-4 mb-3 transition-all duration-200 hover:shadow-md hover:border-gray-300 {contentTypeClass}">
  <div class="flex items-center justify-between mb-2">
    <span class="inline-block px-2 py-1 text-xs font-medium rounded-full uppercase tracking-wide {getBadgeClass(block.type)}">
      {getTypeLabel(block.type)}
    </span>
    {#if showUUID}
      <code class="text-xs text-gray-500 font-mono bg-gray-100 px-2 py-1 rounded">{block.id}</code>
    {/if}
  </div>
  
  <div class="card-content">
    {#if block.type === 'header'}
      <h3 class="text-lg font-bold text-purple-900 leading-tight">{displayContent}</h3>
    {:else}
    ...
    {#if shouldTruncate}
      <button 
        ...
        class="mt-2 text-xs text-blue-600 hover:text-blue-800 font-medium focus:outline-none"
      >
  ...
  {#if showMetadata && block.originalClass}
    <div class="card-metadata">
      ...
      {#if block.metadata && Object.keys(block.metadata).length > 0}
        <div class="metadata-details text-xs text-gray-400 mt-1">
