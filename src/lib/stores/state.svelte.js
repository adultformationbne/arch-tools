	...
	updateFilteredChapters() {
		...
		if (this.searchQuery.trim()) {
			...
			chapters = chapters.filter((chapter) => {
				...
				return chapter.blocks.some((block) => block.content.toLowerCase().includes(query));
			});
		...
		if (this.contentTypeFilter !== 'all') {
			chapters = chapters
				.map((chapter) => {
					const filteredBlocks = chapter.blocks.filter(
						(block) => block.type === this.contentTypeFilter
					);
					return {
						...chapter,
