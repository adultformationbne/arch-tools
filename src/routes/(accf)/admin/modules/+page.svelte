<script>
	...
	const loadSessionData = async (session) => {
		...
		try {
			...
			const materialsResponse = await fetch(`/api/module-materials?module_id=${selectedModule.id}&session_number=${session}`);
			...
			const reflectionResponse = await fetch(`/api/module-reflection-questions?module_id=${selectedModule.id}&session_number=${session}`);
	...
	const handleReflectionChange = async (newReflection) => {
		...
		try {
			...
			const existingQuestions = await fetch(`/api/module-reflection-questions?module_id=${selectedModule.id}&session_number=${selectedSession}`);
			...
			if (existingQuestion) {
				...
				response = await fetch('/api/module-reflection-questions', {
					method: 'PUT',
							...
							>
								<div class="flex items-start justify-between">
									...
									<div
										...
										class="ml-2 p-1 opacity-0 group-hover:opacity-100 text-white/60 hover:text-white transition-all cursor-pointer"
										title="Edit module"
									>
