...
export const load: PageServerLoad = async ({ request }) => {
	...
	let cohortId = '2cb2b8a1-d2f2-4994-b5ca-ede68b29c231'; // fallback
	...
	try {
		...
		materials?.forEach(material => {
			const sessionNum = material.module_sessions.session_number;
		...
		reflectionQuestions?.forEach(question => {
			const sessionNum = question.module_sessions.session_number;
		...
		const processedPublicReflections = publicReflections?.map(response => {
			...
			return {
				...
				cohortName: cohort.modules?.name || 'Unknown Module',
				sessionNumber: response.module_reflection_questions?.session_number || response.session_number || 0,
		...
		const courseData = {
			title: cohort.modules?.name || "Foundations of Faith",
			description: cohort.modules?.description || "",
